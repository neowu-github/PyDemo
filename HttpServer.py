...
try:
    ...
    print '-------->> Warning: Port is not given, will use deafult port: 7070 '
...
if not 1024 < port < 65535:  port = 7070
serveraddr = ('', port)
...
# #             self.send_header("Set-Cookie","sessionid=11893184297152216680; path=/fp-setup; Max-Age=1")
#         cookie = Cookie.SimpleCookie()
#         cookie['sessionid'] = '11893184297152216680'
...
#         self.end_headers()
    ...
    def do_POST(self):
        ...
        st = time.clock()
#         self.sap = drm.FairPlaySAP("airtunesd")
        et = time.clock()
        ...
        if self.path == "/fp-setup":
            ...
            print 'fpsetup post:',data
            response = 'ddddddd'
#             if data.keys().__len__() == 1 and 'fpsetup1' in data.keys():
#                 s1 = base64.b64decode(data['fpsetup1']) 
#                 response = self.sap.challenge(3, s1, 0)
#             elif data.keys().__len__() == 2:
#                 s1 = base64.b64decode(data['fpsetup1'])
#                 s2 = base64.b64decode(data['fpsetup2'])
#                 self.sap.challenge(3, s1, 0)
#                 response = self.sap.challenge(3, s2, 1)
#             else:
#                 response = "data error"
            self.send_response(200)
            self.send_header("Date", self.date_time_string())
...
#             self.wfile.write('\r\n')
            ...
            data = self.rfile.read(int(self.headers["Content-Length"]))
#             data = json.loads(data)
            print 'stream post:',data
#             s1 = base64.b64decode(data['fpsetup1'])
#             s2 = base64.b64decode(data['fpsetup2'])
#             p1 = base64.b64decode(data['stream'])
#             self.sap.challenge(3, s1, 0)
#             self.sap.challenge(3, s2, 1)
#             st = time.clock()
#             key = self.sap.decrypt_key(p1)
#             et = time.clock()
#             print "key:[",key.encode("hex"),"]",type(key),key
...
#             response = key
            ...
            self.send_header("Content-Type", "text/x-apple-plist+xml")
            self.send_header("Content-Length", str(len(response)))
#             self.send_header("Set-Cookie","sessionid=11893184297152216680; path=/stream; Max-Age=1")
...
#             self.wfile.write('\r\n')
    ...
    def deal_post_data(self, data):
        return 'test <%s> test' % data
        pass
